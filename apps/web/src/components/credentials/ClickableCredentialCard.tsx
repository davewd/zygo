...
const mapIssuingBodyToProviderId = (issuingBody: string): string | null => {
  ...
  let provider = CREDENTIAL_PROVIDERS.find((p) => p.name.toLowerCase() === normalizedIssuingBody);

  if (provider) return provider.id;

  // Find provider by abbreviation match
  provider = CREDENTIAL_PROVIDERS.find(
    (p) => p.abbreviation?.toLowerCase() === normalizedIssuingBody
  );
